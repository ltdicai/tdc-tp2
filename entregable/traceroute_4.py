...
for ttl, respuestas in muestras.items():
    ...
    for ip, rtts in respuestas.items():
        ...
        rtt_outliers = set(thompson_tau_test(rtts))
        rtts_filt = [item for idx, item in enumerate(rtts) if idx not in rtt_outliers]
        rtt_promedio_filt = sum(rtts_filt)/len(rtts_filt)
        muestreo[ttl].append({
            'ip': ip,
            'cant_respuestas': len(rtts),
            'rtts':rtts,
            'rtt_promedio': rtt_promedio,
            'rtts_filt': rtts_filt,
            'rtt_promedio_filt': rtt_promedio_filt,
        })
...
for ttl, respuestas in muestreo.items():
    ...
    if len(respuestas) == 1:
        ...
        rtts = respuesta['rtts']
        rtt_promedio_filt = respuesta['rtt_promedio_filt']
        rtts_filt = respuesta["rtts_filt"]
    else:
    ...
    camino.append({
        ...
        'ip': ip,
        'rtt': rtt_promedio,
        'rtt_filt': rtt_promedio_filt,
        'pais': pais
    })

rtt_relativos = list()
rtt_relativos_filt = list()
camino_aux = [(item["ip"], item["rtt"], item["ttl"], item["rtt_filt"]) for item in camino if item["ip"] is not None]
ip_relativos = list()
saltos = list()
for i in xrange(1, len(camino_aux)):
    ...
    rtt_relativos_filt.append(abs(destino[3] - origen[3]))
...
outliers_filt = thompson_tau_test(rtt_relativos_filt)
...
with open(hostname + "_muestreo.csv", "w+") as muestreofile:
    ...
    for ttl, respuestas in muestreo.items():
        temp = { ttl: [
            {
                'ip': respuesta['ip'],
                'cant_respuestas': respuesta['cant_respuestas'],
                'rtt_promedio': respuesta['rtt_promedio'],
                'rtt_promedio_filt': respuesta['rtt_promedio_filt']
            }
            for respuesta in respuestas
...
with open(hostname + "_saltos.csv", "w+") as saltosfile:
    ...
    for x in xrange(len(ip_relativos)):
        ...
        marcador2 = x in outliers_filt and '[outlier_f]' or ''
        temp = "({4:>2d}) {0:<20s} -> ({5:>2d}) {1:<20s}\t{2}\t{3}\t{6}".format(obtener_pais(ip_relativos[x][0]),
